...
- **`release-node.yml`** — Manual dispatch. Takes a package selector (`hdb`, `hdbext`, or `both`) and semver version input. Tests, generates types, bumps `package.json` + `npm-shrinkwrap.json`, publishes to npm with provenance, commits, and creates `hdb/vX.Y.Z` / `hdbext/vX.Y.Z` tags. Requires `NPM_TOKEN` secret.
- **`release-python.yml`** — Manual dispatch. Takes a semver version input, validates, tests, bumps `pyproject.toml` version, builds sdist+wheel, commits, creates tag `hdbhelper-py/vX.Y.Z`, and publishes to PyPI via OIDC trusted publishers. Requires a `pypi` GitHub Environment.
